...
#filename = "/unix/qtnm/sjones/outputs/WaveguideSignalGen/v2/out_0622e416-6787-419f-906b-2f89ae2a5765.h5"
#filename = "/unix/qtnm/sjones/outputs/WaveguideSignalGen/v2/out_c68b6b4f-1df3-4c55-94da-58e33d1ad652.h5"
#filename = "/unix/qtnm/sjones/outputs/WaveguideSignalGen/v2/out_0622e416-6787-419f-906b-2f89ae2a5765.h5"
startFs=[]
houghstartFs=[]
zmaxs=[]
#filename = "/unix/qtnm/sjones/outputs/WaveguideSignalGen/v3/out_04974749-daa1-48f2-b069-82728a64cd84.h5"
...
for filename in filenames:
    ...
    #File can be opened
        ...
        if f:
            ...
            if group:
                ...
                # Loop over datasets in group islice(group.keys(), 3)
        ...
        axis=-1, mode='psd')
                            ...
                            # Convert startFdown to bins
                            start_bin = startFdown / delta_f
                            ...
                            # Make binary image
                            spectrogram[spectrogram > power_threshold] = 1

                            # time scale factor
                            time_scale = int(np.round(len(freq)/len(time)))
                            

                            # Make a reasonable sized spectrogram
                            image = np.repeat(spectrogram, time_scale, axis=1)

                            #stretch_time = np.linspace(time[0], time[-1], len(time)*time_scale, endpoint=True)

                            # Initialize an empty list to store stretched time segments
                            stretch_time= []

                            # Iterate over each pair of consecutive time values
                            for i in range(len(time)-1):
                                # Divide the interval between consecutive time values into 546 evenly spaced segments
                                stretched_segment = np.linspace(time[i], time[i + 1], time_scale+1)
                                stretch_time.append(stretched_segment[:-1])  # Exclude the last element to avoid duplicates
                            
                            # Concatenate the stretched time segments into a single array
                            time_first = np.linspace(0, time[0], time_scale, endpoint=False)
                            
                            stretch_time = np.concatenate([time_first]+stretch_time)
                            
    ##
    ##                        plt.figure(dpi=200)
    ##                        plt.pcolormesh(time*1e6, freq*1e-6, spectrogram, shading='gouraud', cmap='bone_r')
    ##                        plt.colorbar(label='Power/Frequency')
    ##                        plt.title('Spectrogram for {}'.format(dsetName))
    ##                        plt.ylabel('Frequency band (MHz)')
    ##                        plt.xlabel('Time window (microseconds)')
    ##                        plt.show(block=False)

                            #plt.figure(dpi=200)
                            #plt.pcolormesh(stretch_time/max(time), freq/max(freq), image, shading='gouraud', cmap=cm.gray)
                            #plt.colorbar(label='Power/Frequency')
                            #plt.title('Spectrogram for {}'.format(dsetName))
                            #plt.ylabel('Frequency band (MHz)')
                            #plt.xlabel('Time window (microseconds)')
                            ...
                            # noise temp values to test
                            noise_temps = [0]

                            # plot for different noise temps and use B=500MHz
                            # think about efficiency of detector, only travelling in one direction of waveguide
                            # extract actual electron energy spectrun
                            # Kurie plot fitting        
                                                

                            # Plot the original and noisy spectrograms for different SNR values
    ...
    ##                        plt.title('Spectrogram for {}'.format(dsetName))
                            ...
                            for temp in noise_temps:
                                ...
                                noisy_data, sigma = add_thermal_noise(np.array(dset), temp)
                                ...
                                # Make all lower powers 0
                                power_threshold = (np.amax(spectrogram)-np.amin(spectrogram))*0.3  # Adjust the threshold value as needed
                                # Set values below the power threshold to zero
                                spectrogram[spectrogram < power_threshold] = 0
                                ...
                                lines = probabilistic_hough_line(image, threshold=1500, line_length=800,
                                         line_gap=3)
    ...
    ##                            ax = axes.ravel()
    ##
    ##                            ax[0].imshow(spectrogram, cmap=cm.gray)
    ##                            ax[0].set_title('Input image')
    ##
    ##                            ax[1].imshow(image, cmap=cm.gray)
    ##                            ax[1].set_title('Canny edges')
    ##
    ##                            ax[2].imshow(spectrogram)

                                min_distance = float('inf')
                                       
                                for line in lines:
                                    # p0 will contain the values (xstart, ystart) and p1 will contain the values (xend, yend)
                                    p0, p1 = line
    ##                                ax[2].plot((p0[0], p1[0]), (p0[1], p1[1]))
                                    
                                    if p0[0]==0:

                                        # Calculate the distance between the start frequency and start_bin
                                        distance = abs(p0[1] - start_bin)
                                        ...
                                        if distance < min_distance:
                                            ...
                                            # Update the closest start frequency and its corresponding line
                                            closest_start_frequency = p0[1]
                                            
                                            

                                if 'closest_start_frequency' in locals() or 'closest_start_frequency' in globals():
                                    print(f'The start frequency = {closest_start_frequency*delta_f*1e-6} and real start bin = {startFdown*1e-6}')
                                    
                                    # Append the values to the lists
                                    #startFs.append(startFdown*1e-6)
                                    houghstartFs.append(closest_start_frequency*delta_f*1e-6)
                                    zmaxs.append(zmax)
                                    
                                else:
                                    # Append the values to the lists
                                    #startFs.append(startFdown*1e-6)
                                    houghstartFs.append(0)
                                    zmaxs.append(zmax)
    ...
    ##                            ax[2].scatter(0, start_bin, color='red')
...
print (f' The true start frequencies = {startFs} and the hough predicted ones are {houghstartFs}')
print (f'zmax={zmaxs}')
...
# Use a high pass filter to determine frequency bins to look in, then just look in these bins for linear signals 
